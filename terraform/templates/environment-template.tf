...
# This template eliminates duplication across dev/staging/prod
...
# Key Vault (only for staging/prod)
module "keyvault" {
  ...
  access_policies = var.environment == "dev" ? [
    {
...
# Databases (only for staging/prod)
module "databases" {
  count = var.environment != "dev" ? 1 : 0
